...
void UMatrix::AdjustScoreToZero(int    startingAt,
                                int    numberOfColumns)
{
...
void UMatrix::AdjustColumn(int       column,
                           double     adjustment)
{
...
double UMatrix::PreviousScore(int     row,
                             int     d)
{
    double s = NEG_INF;
...
// Account for warpping if currentColumnIndex - d < 0
int UMatrix::PreviousTraceback(int    row,
                               int    d)
{
    int r = 0;
    if (d < length) {
 ...
 */
int *UMatrix::ForwardTraceback(int    *traceArray,
                               int    windowLength,
                               int    row)
{
    // The traceArray is forward oriented, and has correct insertion and deletion positions
    if (windowLength > length) {
        printf("(ForwardTraceback) Length greater than matrix length, ret NULL\n");
        return NULL;
    }
    
    // If traceArray is NULl then we need to allocate the memory for our traceArray
    if (traceArray == NULL) {
        traceArray = (int *)malloc(sizeof(int) * (windowLength + 1));
    }
    
    int pos = previousColumnIndex; // pos is where in the tracebackMatrix we are looking
    ...
    for (int i = 0; i <= windowLength; ++i) {
        ...
        // In case of C_INSERTION we need to set the next (order + indelnum) characters to
        // be the cell of the insertions parent cell
        // and then set the last character to the insertion. We do things in this order
        // because insertions are detected (order + indelnum) after they occur, and we are reading
        // the traceback matrix in reverse order.
        ...
        // Whenever we decrease pos we need to check and make sure we haven't gone past the matrix bounds
        // if we have then we need to adjust for that
        pos = pos - 1;
        if (pos < 0) {
            pos = length - 1;
        }
...
UMatrix::UMatrix(int    maximumPeriod,
                 int    maximumInsertions,
                 int    maximumDeletions,
                 int    matrixLength)
{
