...
void UModel::CalculateScores() {
    ...
    tscore[CT_MATCH][CT_MATCH] = log2(1.0 - (tp_matchToZero + tp_matchToInsertion + tp_matchToDeletion));
    ...
    /*if (adjustMatchMismatchMatrix) {
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            //  printf("%f %f %f %f\n", backgroundScores[i][N_A], backgroundScores[i][N_T], backgroundScores[i][N_C], backgroundScores[i][N_G]);
            
            for (int x = 0; x < 5; ++x) {
                double s = 0.0;
                for (int y = 0; y < 5; ++y) {
                    matchScores[i][x][y] = log2(backgroundScores[i][x] * backgroundScores[i][y] + 0.0001) * matchProbabilities[x][y];
                    s += matchScores[i][x][y];
                }
...
double UModel::PreviousEmissionScore (SequenceWindow *seq,
                                      int    index,
                                      int    order,
                                      int    d)
{
   ...
   // symbol s = seq->seq[p];
    //printf("%i, %i %i %i, %f\n", p, order, p-order, (int)seq->seq[p], mscore[p-order][seq->seq[p]]);
...
double UModel::EmissionScore (SequenceWindow   *seq,
                              int     index,
                              int     order)
{
...
void UModel::CalculateCurrentColumn  (SequenceWindow *sequence,
                                      int      nucIndex,
    ...
    for (int row = 0; row < matrix->cellsPerColumn; ++row) {
        ...
        switch (desc[row].type) {
            ...
            case CT_BACKGROUND:
            {
                ...
                // This does not check insertion->match or deletion->match - those are done in
            ...
            case CT_MATCH:
            {
                ...
                // Before we calculate normal match scores we need to check to see if we have had enough characters pass
                if (matind < order) {
                    c[row] = NEG_INF;
                    ct[desc[row].order] = 0;
                    break;
                ...
                // Testing out having transitions to state n come from 0th order n characters previous
            ...
            case CT_INSERTION:
            {
              ...
              //  int indelNum = desc[row].indelNumber;
                ...
                if (matind > minIndex) {
                    ...
                    double score = p[row] + tscore[CT_INSERTION][CT_MATCH] + EmissionScore(sequence, nucIndex, order);
                    if (score > c[parentIndex]) {
                        // printf("%i %i %f %f\n", order, indelNum, score, c[parentIndex]);
                        c[parentIndex] = score;
                        ct[desc[row].order] = row;
                        
                        //  printf("in[%i for %i:%i,%i,%i,%i] :%f vs %f to p %f (%f, %f= (%f with %f)\n", row, nucIndex, minIndex, parentIndex, order, indelNum, p[row], score, c[parentIndex], d, f,  matrix->PreviousScore(parentIndex, peridocity),  matrix->PreviousScore(parentIndex, peridocity + 1));
                ...
                // We have to do a manual calculation of the score
                    ...
                    double score = matrix->PreviousScore(parentIndex, peridocity); // The match score this insertion comes from
                    score += 0.0; // The emission score of the actual insertion = 0 = bscore[n] - bscore[n]
                    
                    // Transition cost
                    score += tscore[CT_MATCH][CT_INSERTION];
                    score += tscore[CT_INSERTION][CT_INSERTION] * (double)(desc[row].indelNumber - 1);
                    score += log2(1.0 - tp_matchToZero) * (double)order;
                    
                    // We don not need to consider the insertion cost at this point because
                    // the insertion state is 0th order, and will have emission score = 1 = bscore/bscore = e^0
            ...
            case CT_DELETION:
            {
                ...
                if (matind > minIndex) {
                    ...
                    double score = p[row] + tscore[CT_DELETION][CT_MATCH] + EmissionScore(sequence, nucIndex, order);
                    if (score > c[parentIndex]) {
                        c[parentIndex] = score;
                        ct[desc[row].order] = row;
                        
                        
                        // printf("del[%i for %i:%i,%i,%i,%i] :%f vs %f to p %f (%f, %f= (%f with %f)\n", row, nucIndex, minIndex, parentIndex, order, indelNum, p[row], score, c[parentIndex], d, f,  matrix->PreviousScore(parentIndex, order),  matrix->PreviousScore(parentIndex, order + 1));
                    }
...
UModel::UModel(UMatrix  *m) {
    this->matrix = m;
}

UModel::UModel(int maxPeriod,
               int maxInsertions,
               int maxDeletions,
               int matrixLength)
{

    this->matrix = new UMatrix(maxPeriod, maxInsertions, maxDeletions, matrixLength);
    CalculateScores();
