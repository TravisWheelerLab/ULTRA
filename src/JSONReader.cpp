    ...
    if (i % maxSeqLength == 0) {
      ...
      if (i > maxOverlapLength) {
        for (int x = 0; x < maxOverlapLength; ++x) {
          window->seq[p++] =
              SymbolForChar(r->sequence[i - maxOverlapLength + x]);
        }
        window->overlap = p;
      }
    }
...
SequenceWindow *JSONReader::GetWaitingWindow() { return NULL; }

bool JSONReader::AddWaitingWindow(SequenceWindow *window) { return true; }

JSONReader::JSONReader(std::string fp, unsigned long maxSeqLeng,
