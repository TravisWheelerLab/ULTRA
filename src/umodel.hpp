...
public:
  ...
  double matchProbabilities[5][5]{{1.0, .3, .2, .3, .2},           // [N][*]
  ...
  void SetATCGProbabilities(double A, double T, double C, double G);

  void CalculateScores(); // Needs to be called before general use

  double PreviousEmissionScore(SequenceWindow *seq, int index, int order,
                               int d);

  double EmissionScore(SequenceWindow *seq, int index, int order);
